...
{
    public partial class EditTblSmetkaComponent : ComponentBase
    {
        ...
        {
            var bssr4GetTblSmetkaByIdResult = await Bssr4.GetTblSmetkaById(ID);
            tblsmetka = bssr4GetTblSmetkaByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Bssr.Models.Bssr4.TblSmetka args)
        {
            try
            {
                var bssr4UpdateTblSmetkaResult = await Bssr4.UpdateTblSmetka(ID, tblsmetka);
                DialogService.Close(tblsmetka);
            }
            catch (System.Exception bssr4UpdateTblSmetkaException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update TblSmetka");
            }
        }
