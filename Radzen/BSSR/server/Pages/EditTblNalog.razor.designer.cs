...
{
    public partial class EditTblNalogComponent : ComponentBase
    {
        ...
        {
            var bssr4GetTblNalogByIdResult = await Bssr4.GetTblNalogById(ID);
            tblnalog = bssr4GetTblNalogByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Bssr.Models.Bssr4.TblNalog args)
        {
            try
            {
                var bssr4UpdateTblNalogResult = await Bssr4.UpdateTblNalog(ID, tblnalog);
                DialogService.Close(tblnalog);
            }
            catch (System.Exception bssr4UpdateTblNalogException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update TblNalog");
            }
        }
