...
{
    public partial class EditTblDokumentiComponent : ComponentBase
    {
        ...
        {
            var bssr4GetTblDokumentiByIdResult = await Bssr4.GetTblDokumentiById(ID);
            tbldokumenti = bssr4GetTblDokumentiByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Bssr.Models.Bssr4.TblDokumenti args)
        {
            try
            {
                var bssr4UpdateTblDokumentiResult = await Bssr4.UpdateTblDokumenti(ID, tbldokumenti);
                DialogService.Close(tbldokumenti);
            }
            catch (System.Exception bssr4UpdateTblDokumentiException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update TblDokumenti");
            }
        }
