...
{
    public partial class EditTblFakturaComponent : ComponentBase
    {
        ...
        {
            var bssr4GetTblFakturaByIdResult = await Bssr4.GetTblFakturaById(ID);
            tblfaktura = bssr4GetTblFakturaByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Bssr.Models.Bssr4.TblFaktura args)
        {
            try
            {
                var bssr4UpdateTblFakturaResult = await Bssr4.UpdateTblFaktura(ID, tblfaktura);
                DialogService.Close(tblfaktura);
            }
            catch (System.Exception bssr4UpdateTblFakturaException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update TblFaktura");
            }
        }
