...
{
    public partial class EditTblFakturaStavkiComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Bssr.Models.Bssr4.TblFakturaStavki args)
        {
            try
            {
                var bssr4UpdateTblFakturaStavkiResult = await Bssr4.UpdateTblFakturaStavki(ID, tblfakturastavki);
                DialogService.Close(tblfakturastavki);
            }
            catch (System.Exception bssr4UpdateTblFakturaStavkiException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update TblFakturaStavki");
            }
        }
